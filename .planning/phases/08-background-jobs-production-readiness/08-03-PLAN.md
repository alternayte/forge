...
must_haves:
  truths:
    ...
    - "Event type has a Type field and CloseEvent sentinel exists for SSE-03 graceful shutdown close frame"
  artifacts:
    ...
    - path: "internal/notify/subscription.go"
      provides: "Subscription type with Events channel and cancel function, Event type with Type field, CloseEvent sentinel"
  ...
  key_links:
    ...
    - from: "internal/sse/limiter.go"
      ...
      via: "Caller passes SSEConfig.MaxTotalConnections and SSEConfig.MaxPerUser as int args to NewSSELimiter — limiter does not import config directly"
      pattern: "NewSSELimiter"
---
...
**SSE-03 (graceful shutdown with close event):** Context cancellation is the mechanism — SSE handlers receive ctx from the HTTP request. When server calls Shutdown(), contexts are cancelled, SSE goroutines exit, release() is called. No special shutdown logic needed in the limiter itself. However, the SSE handler must send a close event before ending the connection so clients can distinguish graceful shutdown from network errors. The limiter does not handle this directly — the SSE HTTP handler (wired in a later plan) is responsible for detecting ctx.Done() and writing `event: close\ndata: {}\n\n` before returning.
...
1. Create `internal/notify/subscription.go` with package `notify`:
   - `Event` struct with `Channel string`, `Payload []byte`, and `Type string` fields. The `Type` field defaults to empty (normal data event). When `Type == "close"`, the SSE handler should write `event: close\ndata: {}\n\n` and terminate the connection (SSE-03 graceful shutdown).
   - Export a sentinel: `var CloseEvent = Event{Type: "close"}` for use by the SSE handler when ctx is cancelled.
