...
**Size target (DEPLOY-02):** The binary size < 30MB is achieved by `-trimpath -ldflags="-s -w"` which strips debug info and symbol table. Typical Go binaries with embedded assets (migrations ~200KB + static ~500KB) are well under 30MB. No additional work needed beyond the build flags. **Verification:** After `go build`, print the binary size using `os.Stat()` and log a warning if it exceeds 30MB — this serves as a runtime assertion during development. The `forge build` output already prints binary size (see Task 1 step d).

**Startup time (DEPLOY-03):** Go binaries start natively — no JIT, no reflection-heavy init. pgxpool lazy-connects by default. River client starts goroutines asynchronously. The < 100ms cold start is an architecture guarantee of Go's native compilation. **Verification:** This is not measured at build time. It is an inherent property of Go's runtime (no VM warmup, no class loading). If startup latency becomes a concern, it can be measured with `time ./bin/server --help` during integration testing.
  </action>
  <verify>
Run `go build ./internal/cli/...` — compiles.
...
7. DEPLOY-02: `forge build` prints binary size; verify it is under 30MB (architecture guarantee: Go binary with -s -w + embedded assets)
8. DEPLOY-03: startup < 100ms is a Go runtime architecture guarantee (native compilation, no VM); verify with `time ./bin/server --help` if needed
</verification>
