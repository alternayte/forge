...
3. Add a funcmap helper `hasHooks` in `internal/generator/funcmap.go` that returns true when either AfterCreate or AfterUpdate is non-empty, and register it in `BuildFuncMap()`. This will be needed by the actions template in Plan 04.
  </action>
  <verify>
...
Run `go vet ./internal/parser/...` — no issues.
  ...
  <done>
`JobRefIR` and `HooksIR` types exist in ir.go. `ResourceOptionsIR.Hooks` field exists. Parser extracts `WithHooks` calls from schema files into `HooksIR` with correct Kind/Queue values. `hasHooks` funcmap helper registered. All existing parser tests pass without regression.
  </done>
</task>
