...
must_haves:
  truths:
    ...
    - "Form template conditionally renders fields based on role using Visibility/Mutability modifiers from the IR"
  artifacts:
  ...
  key_links:
    ...
    - from: "scaffold_list.templ.tmpl"
      ...
      pattern: "models\\."
    - from: "scaffold_form.templ.tmpl"
      to: "role-based visibility"
      via: "if guards around fields with Visibility modifier"
      pattern: "role.*==|hasVisibility"
---
...
<task type="auto">
  ...
  <action>
    ...
    3. Component signature: `templ {{.Resource.Name}}Form({{lower .Resource.Name}} *models.{{.Resource.Name}}, errors map[string]string, role string)`
       - The role parameter is used for conditional field rendering (HTML-07).
    ...
    5. For each non-ID field in .Resource.Fields (skip ID fields using isIDField), add role-based visibility guards:

       The Go text/template iterating over fields must check each field's Modifiers for a modifier with Type "Visibility". Use a funcmap helper `hasModifier` (or inline range check) to detect this.

       **Fields WITHOUT a Visibility modifier:** Render unconditionally (visible to all roles).

       **Fields WITH a Visibility modifier:** Wrap the field rendering in an if-guard in the generated Templ output. The Visibility modifier's Value is the role name string that is allowed to see the field. Generate a Templ `if` block:
       ```
       if role == "" || role == "<visibility-value>" {
         // render field
       }
       ```
       The `role == ""` fallback ensures fields render when no role context is provided (e.g., admin views, development). This is a simple equality check for Phase 6. Phase 7 may introduce hierarchical role matching via the permissions system, but the scaffold provides the conditional framework now.

       **Fields WITH a Mutability modifier:** Similarly check before rendering the input as editable vs read-only. If the role does not match the Mutability modifier's Value, render the field value as a read-only span/text instead of an input. If the role matches (or role is empty), render the normal editable input.

       For each visible+editable field:
    ...
    To implement the modifier checks in Go text/template, add a `hasModifier` funcmap helper to funcmap.go (or use an inline `{{range .Modifiers}}{{if eq .Type "Visibility"}}` pattern). The helper approach is cleaner:
    - `hasModifier(field FieldIR, modType string) bool` — returns true if field has modifier of given type
    - `modifierValue(field FieldIR, modType string) string` — returns the Value as string for the first modifier of given type
  ...
  <done>
    scaffold_form.templ.tmpl generates Datastar-native form with data-bind inputs per field, field-level error display, role-based conditional visibility/mutability guards around fields with Visibility or Mutability modifiers, and import of primitives. Fields without modifiers render unconditionally. Fields with Visibility modifier are wrapped in role-check if-guards. Fields with Mutability modifier render as read-only when role doesn't match. scaffold_detail.templ.tmpl generates read-only detail view with definition list layout.
  </done>
</task>
...
- scaffold_form.templ.tmpl contains: role-based if-guards for fields with Visibility modifiers (checking hasModifier or inline range on .Modifiers)
- scaffold_form.templ.tmpl contains: mutability check that renders read-only display for fields where role doesn't match Mutability modifier
...
- HTML-07 satisfied: Form conditionally renders fields based on role using Visibility/Mutability modifier if-guards (simple role equality check; hierarchical roles deferred to Phase 7 permissions system)
</success_criteria>
