...
must_haves:
  ...
  artifacts:
    ...
    - path: "internal/generator/funcmap.go"
      provides: "hasPermission, permissionRoles, hasAnyVisibility funcmap helpers"
  ...
  key_links:
    ...
    - from: "internal/generator/templates/actions.go.tmpl"
      ...
      via: "Template uses hasPermission/permissionRoles to emit conditional permission checks"
...
Output: Updated actions.go.tmpl with permission guards and field visibility filtering. New internal/auth/context.go with user/role context helpers. Updated funcmap.go with permission template helpers.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    4. Ensure the funcmap helpers from Plan 01 are complete. In `funcmap.go`, verify `hasAnyVisibility`, `hasPermission`, `permissionRoles`, and `hasAnyPermission` are all registered in BuildFuncMap() and have correct implementations. If Plan 01 already added them, this is a verification step. If any are missing, add them now.

       `permissionRoles` should return a string suitable for embedding in Go code as variadic args:
       ```go
  ...
  <done>
    roleFilter method omits fields with Visibility modifier when user role doesn't match. RoleFilterList method filters a slice of items. Permission and visibility funcmap helpers are complete and registered. API handlers can call RoleFilterList to strip invisible fields before returning responses.
  </done>
</task>
...
7. funcmap has hasPermission, permissionRoles, hasAnyVisibility, hasAnyPermission
</verification>
