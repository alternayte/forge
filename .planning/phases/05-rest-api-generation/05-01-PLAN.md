...
must_haves:
  ...
  artifacts:
    ...
    - path: "internal/generator/templates/api_register.go.tmpl"
      provides: "Huma route registration calling action layer"
...
Run `go test ./internal/generator/ -run TestGenerateAPI -v` to verify all tests pass. Run `go build ./internal/generator/` and `go vet ./internal/generator/` for clean compile.
  ...
  <done>
Route registration template generates 5 CRUD endpoints per resource with correct Huma OperationIDs, paths (/api/v1/{kebab-plural}), tags, and thin handlers calling action layer. GenerateAPI function produces gen/api/ directory with inputs, outputs, routes, and types files per resource. All tests pass.
  </done>
</task>
...
- New funcmap helpers (kebab, lowerCamel, humaValidationTag) registered and tested
</success_criteria>
