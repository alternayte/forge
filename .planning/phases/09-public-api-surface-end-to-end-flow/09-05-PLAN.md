...
func main() {
	...
	registry := genactions.NewRegistry()
...
**CONFIRMED: `NewRegistry()` takes no arguments.** The generated `actions.go.tmpl` produces `NewRegistry()` with zero parameters. No DB pool or other arguments are needed at construction time. The template line above is correct as written.
