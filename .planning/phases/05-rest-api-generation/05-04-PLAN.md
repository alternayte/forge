...
must_haves:
  truths:
    ...
    - "Exported OpenAPI spec passes spectral linting for SDK generation readiness"
  artifacts:
...
<task type="auto">
  <name>Task 2: Create forge openapi export command with JSON/YAML format flag and spectral lint verification</name>
  <files>
    internal/cli/openapi.go
    internal/cli/root.go
  </files>
...
6. **Spectral lint readiness** (API-10): The exported spec must pass spectral linting and produce working SDKs. Ensure:
   - Every operation has an explicit operationId (verb+noun camelCase)
   - Every operation has tags
   - Every operation has a summary
   - Response schemas include proper data types (not empty objects)
   - No duplicate operationIds across the entire spec
...
- openapiCmd.AddCommand(openapiExportCmd)
  ...
  <verify>
Run `go build ./cmd/forge/` to confirm the CLI compiles. Run `go vet ./internal/cli/`. Verify `forge openapi --help` shows export subcommand. Verify `forge openapi export --help` shows --format flag. If spectral is available (npx @stoplight/spectral-cli), run `forge openapi export openapi.json && npx @stoplight/spectral-cli lint openapi.json` to verify the exported spec passes linting. If spectral is not installed, verify the spec is valid JSON and contains operationId for every operation by inspecting the output with `jq '.paths[][].operationId' openapi.json | sort -u`.
  </verify>
  <done>
forge openapi export command builds OpenAPI spec from parsed schemas and writes to file. Supports --format json|yaml per user decision. Creates in-memory Huma API from IR for spec generation. Every operation has explicit operationId, tags, and summary for SDK readiness. Command registered as subcommand of forge openapi. Exported spec is spectral-lint ready with no missing operationIds or duplicate IDs.
  </done>
</task>
...
8. Exported OpenAPI spec contains operationId for every operation (verify with jq or spectral lint)
9. If spectral available: `npx @stoplight/spectral-cli lint openapi.json` passes with no errors
</verification>
...
- Exported spec has operationId, tags, and summary on every operation (SDK and spectral lint ready)
