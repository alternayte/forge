...
must_haves:
  truths:
    ...
    - "Eager-loaded relationships exclude soft-deleted related records"
  artifacts:
    ...
    - path: "internal/generator/templates/actions.go.tmpl"
      provides: "recordAudit helper, computeJSONDiff, audit calls in Create/Update; eager loading logic"
  ...
  key_links:
    ...
    - from: "internal/generator/templates/actions.go.tmpl"
      to: "RelationshipIR.Eager"
      via: "Eager relationships trigger batch loading with soft-delete filter"
      pattern: "Eager"
---

<objective>
Implement audit logging (JSONB diffs, created_by/updated_by auto-population, no-op detection), the audit_logs table, audit API endpoint, and eager relationship loading with soft-delete awareness.

Purpose: AUDIT-01 through AUDIT-03 require complete change tracking for Auditable resources. DATA-12 requires that eager-loaded relationships automatically exclude soft-deleted records. This plan completes the remaining Phase 7 requirements.

Output: Updated model, atlas, actions, and api_register templates with audit and eager loading logic.
</objective>
...
<task type="auto">
  <name>Task 2: Add audit recording to actions and eager loading with soft-delete awareness</name>
  <files>
    internal/generator/templates/actions.go.tmpl
    internal/generator/templates/api_register.go.tmpl
  </files>
  <action>
    ...
    4. For DATA-12 (Eager loading with soft-delete), add batch loading in the actions template. After the List method's main query section, add eager loading logic guarded by relationship Eager flag:

       ```
       {{- range .Relationships}}
       {{- if .Eager}}
       // Eager-load {{.Name}} relationships (batch query to prevent N+1)
       // Note: If related resource has SoftDelete, the batch query filters deleted_at IS NULL
       // to automatically exclude soft-deleted related records (DATA-12).
       // TODO: Implement batch loading when Bob query execution is complete.
       // Pattern: SELECT * FROM {{.Table}} WHERE {{snake $.Name}}_id = ANY($1) AND deleted_at IS NULL
       {{- end}}
       {{- end}}
       ```

       Since the action layer still has TODO placeholders for actual query execution (Bob integration), eager loading is documented as a TODO with the correct query pattern. The relationship IR now has the Eager flag (from Plan 01), so when Bob queries are implemented, the eager loading logic will be generated here.
  </action>
  <verify>
    Run `cd /Users/nathananderson-tennant/Development/forge-go && go build ./internal/generator/...` — compiles. Review actions.go.tmpl for recordAudit, computeJSONDiff, audit calls in Create/Update/Delete. Review api_register.go.tmpl for audit endpoint. Review atlas_schema.hcl.tmpl for audit_logs table.
  </verify>
  <done>
    recordAudit method records JSONB diffs to audit_logs table. computeJSONDiff computes field-level before/after changes. Create records all initial values as first audit entry. Update fetches before-state and records diff; no-op updates produce no audit entry (AUDIT-03). Delete records soft-delete in audit log. API endpoint GET /api/v1/{resource}/:id/audit returns audit entries. GetDB method exposed on DefaultActions for audit queries. Eager loading pattern documented in actions template with soft-delete filter for related records (DATA-12).
  </done>
</task>
...
8. Eager loading TODO pattern includes deleted_at IS NULL for soft-delete awareness
</verification>

<success_criteria>
Audit logging is fully wired: created_by/updated_by auto-populated, JSONB diffs recorded for all changes, no-op updates skipped, audit_logs table generated once, and API endpoint exposes audit history. Eager loading is documented with soft-delete aware batch query pattern (DATA-12). All Phase 7 requirements (SCHEMA-07, SCHEMA-08, DATA-05 through DATA-09, DATA-12, AUTH-05, AUTH-06, TENANT-01 through TENANT-04, AUDIT-01 through AUDIT-03) are addressed across Plans 01-05.
</success_criteria>
